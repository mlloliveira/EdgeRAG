#!/usr/bin/env python3
"""
edge_rag_final_analysis_v2_3.py
...
def normalize_text(s: str) -> str:
    """SQuAD-style normalization for EM/F1 (matched to the runner)."""
...
def compute_exact_match(pred: str, golds: List[str]) -> int:
    """Exact match against any gold answer (matched to the runner)."""
...
def compute_f1(pred: str, golds: List[str]) -> float:
    """Token-level F1 (max over golds), SQuAD-style (matched to the runner)."""
...
def strip_think_prefix_for_scoring(answer: Any) -> Any:
    """Strip a leading <think>...</think> block before scoring, if present."""
...
def main() -> None:
    ...
    parser.add_argument("--results", type=Path, default=Path("/mnt/data/results.jsonl"))
    parser.add_argument("--config", type=Path, default=None, help="Optional config JSON with model nicknames and order")
    parser.add_argument("--outdir", type=Path, default=Path("final_results_analysis"))
    ...
    with summary_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"Config used: {args.config if args.config else 'None'}\n")
