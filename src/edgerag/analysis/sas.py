...
This script wraps the current analysis pipeline in base.py and adds
...
from edgerag.analysis.common import SAS_ANALYSIS_OUTDIR, resolve_config_path, resolve_results_path


def _load_base_module(_base_path: Path | None = None):
    return base_module
...
def run_self_tests() -> None:
    ...
    base = _load_base_module(Path(__file__).with_name('base.py'))
    raw = pd.DataFrame([
        {
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description='EdgeRAG analysis with optional SAS extension')
    p.add_argument(
        '--results',
        type=Path,
        default=None,
    ...
    p.add_argument('--outdir', type=Path, default=SAS_ANALYSIS_OUTDIR)
    p.add_argument(
        '--base_script',
        type=Path,
        default=None,
        help='Deprecated compatibility argument. The packaged base analysis module is used automatically.',
    )
    ...
    p.add_argument('--skip_sas', action='store_true', help='Skip SAS computation and only run base-analysis outputs plus SAS-shaped tables/figures without scores')
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    if args.base_script is not None:
        warnings.warn('--base_script is deprecated and ignored; the packaged base analysis module is used instead.', stacklevel=2)
    base = _load_base_module(args.base_script)
    config_path = resolve_config_path(args.config)
    results_path = resolve_results_path(args.results, config_path)
    ...
    with summary_path.open('w', encoding='utf-8') as f:
        ...
        f.write('Base analysis module: edgerag.analysis.base\n')
        if args.skip_sas:
            f.write('SAS: skipped\n')
        else:
